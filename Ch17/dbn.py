...
class dbn:
    def __init__(self, nvisible, nhidden=[100, 100, 50], nlabels=0, eta=0.1, momentum=0., nCDsteps=5, nepochs=1000,
                 useprobstate=0):
        self.layers = [
            rbm.rbm(nvisible, nhidden[0], nlabels=None, eta=eta, momentum=momentum, nCDsteps=nCDsteps, nepochs=nepochs)]
        self.nRBMs = len(nhidden) - 1
        for i in range(1, self.nRBMs):
            self.layers.append(
                rbm.rbm(nhidden[i - 1], nhidden[i], nlabels=None, eta=eta, momentum=momentum, nCDsteps=nCDsteps,
                        nepochs=nepochs))
        self.layers.append(
            rbm.rbm(nhidden[self.nRBMs - 1], nhidden[self.nRBMs], nlabels=nlabels, eta=eta, momentum=momentum,
                    nCDsteps=nCDsteps, nepochs=nepochs))
        self.eta = eta
    ...
    def compute_visible(self, hidden, i):
        ...
        self.visibleact = (
        self.visibleprob > np.random.rand(np.shape(self.visibleprob)[0], self.layers[i].nvisible)).astype('float')
    ...
    def compute_hidden(self, visible, i):
        ...
        self.hiddenact = (
        self.hiddenprob > np.random.rand(np.shape(self.hiddenprob)[0], self.layers[i].nhidden)).astype('float')
